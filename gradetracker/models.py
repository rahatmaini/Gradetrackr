...
class Course(models.Model):
    ...
    isIncludedInGPAOrNot = models.BooleanField(default=True) #is this class used in GPA calculation or is it just a fork of a class someone uses purely for "what if" tracking purposes?

    professorEmail = models.EmailField(null=True) #if verified class, then we should be able to get this info 
    avgFromVAgrades = models.DecimalField(max_digits=4,decimal_places=3, null=True) #if verified we should be able to scrape grade from VAgrades.com
